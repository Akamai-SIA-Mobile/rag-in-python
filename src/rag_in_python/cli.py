...
def index(
    ...
    if json_files:
        console.print(f"[blue]Indexing {len(json_files)} JSON files by entries...[/blue]")
        ...
        if all_json_documents:
            ...
            with console.status(f"[bold green]Indexing {len(all_json_documents)} JSON entries..."):
                # Add JSON documents to existing index or create new one
                if hasattr(rag_system, '_index') and rag_system._index is not None:
                    # Add to existing index
                    for doc in all_json_documents:
                        rag_system._index.insert(doc)
                else:
                    ...
                    rag_system._index = VectorStoreIndex.from_documents(
                        all_json_documents,
...
def index_json(
    json_file: Path = typer.Argument(
        ..., 
        help="Path to JSON file to index"
    ),
    index_path: Path = typer.Option(
    ...
    try:
        ...
        # Index documents
        with console.status(f"[bold green]Creating vector index for {len(documents)} entries..."):
        ...
        console.print(f"[green]✓ Successfully indexed {len(documents)} JSON entries to {index_path}[/green]")
...
def query_json(
    ...
    top_k: int = typer.Option(
        5, 
        "--top-k", "-k",
        help="Number of JSON entries to retrieve"
    ),
    ...
    try:
        ...
        # Display metadata
        console.print(f"[dim]Retrieved from JSON entries[/dim]")
...
def list_commands() -> None:
    ...
    commands_info = [
        ...
        ("query-json", "Query indexed JSON entries", "rag-cli query-json 'Find entry about X'"),
    ...
    console.print(f"\n[yellow]💡 Tips:[/yellow]")
    console.print(f"  • Set OPENAI_API_KEY environment variable before using")
    console.print(f"  • Use --help with any command for detailed options")
    console.print(f"  • JSON indexing treats each top-level entry as a separate document")
